...
class CurrencyBox extends React.Component {
  ...
  updateCosts = async () => {
    await this.calculateExchangeRates(this.state.userCurrency);
    ...
    const brazilTransportationCost =
      4100 + 100 * (travelerNumber * nightsNumber);

    //Calculates the user cost in their native currency by multiplying it times USD
    const userHotelPrice = `${(
      (this.props.currency !== "BRL" ? usHotelPrice : brazilHotelCost) *
      this.state.travelerCurrency
    ).toFixed(2)}${this.state.userCurrency}`;
    const userFoodCost = `${(
      (this.props.currency !== "BRL" ? usFoodCost : brazilFoodCost) *
      this.state.travelerCurrency
    ).toFixed(2)}${this.state.userCurrency}`;
    ...
    const userEntertainmentCost = `${(
      (this.props.currency !== "BRL"
        ? usEntertainmentCost
        : brazilEntertainmentCost) * this.state.travelerCurrency
    ).toFixed(2)}${this.state.userCurrency}`;

    const userTransportationCost = `${(
      (this.props.currency !== "BRL"
        ? usTransportationCost
        : brazilTransportationCost) * this.state.travelerCurrency
    ).toFixed(2)}${this.state.userCurrency}`;
